    ...
    public function filter_settings(){
        ...
        if (is_admin()  && isset($_POST['action']) && $_POST['action']=='update' && isset($_POST['option_page']) && $_POST['option_page']==$this->settings_menu['name']) {
            ...
            if (isset ($_POST[$this->settings_menu['name']]['li']) && $_POST[$this->settings_menu['name']]['li'] && (strlen($_POST[$this->settings_menu['name']]['li']) <= 35 || strlen($_POST[$this->settings_menu['name']]['li']) > 40)) {
                $goback = add_query_arg( array('wrong-number'=>'true'),  wp_get_referer() );
                wp_redirect( $goback );
       	        exit;
            }
            ...
            $clean_options_page = remove_query_arg(array('settings-reseted', 'wrong-number', 'settings-imported' ) , wp_get_referer());
    ...
    public function render_option_page() {
        ...
        if ($this->settings_menu['template_file']) {
            ...
            if (isset($_GET['wrong-number']) && $_GET['wrong-number'])
                echo '<div class="error fade"><p><strong>'.__('Purchase code is invalid :-|<br> Read <a target="_blank" href="http://wpwave.com/envato/purchase_code_1200.png">Help</a> or check out <a href="http://www.96down.com" target="_blank">Plugin page</a>.', $this->settings_menu['name']).'</strong></p></div>';
...
'. esc_textarea(stripslashes(json_encode(   array_merge( get_option($this->settings_menu['name']), $empty_keys )  ) )) .'
    ...
    function show_forms() {
            ...
            if ($this->settings_menu['display_metabox'] )
                echo '<div class="metabox-holder" style="width:75%"><div class="postbox">';
            ...
            if (is_network_admin())
               ...
               // do_settings_sections( $this->settings_menu['name']);
                ...
                <span style="padding:0 10px;" class="alignright">
                    <p class="submit">
                        <input name="reset-defaults" onclick="return confirm('<?php _e('Desea borrar las configuraciones y regresar al estado original?', $this->settings_menu['name']); ?>');" class="button-secondary" type="submit" value="<?php _e('Reset Defaults', $this->settings_menu['name']); ?>" />                      </p>
                </span> 
