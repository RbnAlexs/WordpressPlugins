...
/*
Plugin Name: Recent Postbox Cat Widgets
Plugin URI: No public
Description: Muestra la composición del Recent Postbox Cat Widget
Author: M2i Techteam
...
__('Recent Postbox Cat Widget', 'recent_postbox_cat_domain'), 
...
<div class="<?php echo $title ?>">
   ...
   <?php if('on' == $instance['more_in'] ) { ?>
            <div class="morein"> More in <?php $category = get_the_category(); echo $category[0]->cat_name; ?> </div>
         <?php }else { ?>
         <?php } ?>
  ...
  <div  class="recent_cats">  

                <?php if('on' == $instance['thumb'] ) { ?>
                  <div class="thumbs" style="height:<?php echo ($height); ?>px"><a href="<?php the_permalink(); ?>" rel="bookmark" 
                   title="<?php the_title(); ?>">
                 <?php set_post_thumbnail_size( ''. $instance["width"] . ', true' ); ?>
                 <?php the_post_thumbnail();?> 
                 </div>

                <?php } else { ?> <?php }?>

                <div class="recent_cats_title"><a href="<?php the_permalink() ?>"><?php the_title('')?></a></div>  
...
<!-- Parte del more in... -->
<p>
<label for="<?php echo $this->get_field_id( 'more_in' ); ?>"><?php _e( 'Mostrar "more in..."' ); ?></label> 
